...
void Hy3Node::syncHy3Tags() {
	...
	try {
		...
		case Hy3NodeType::Target: {
			...
			// fork: try_window(), not as_window(). This is one of the sites the
			// guard sweep covers - as_window() throws for a target whose weak
			// pointer has expired, and every caller here is a tree mutation
			// reached from a wayland callback. The catch below would stop that
			// terminating the compositor, but there is nothing to catch: a
			// window that is gone simply has no tags to set.
			auto window = this->try_window();
			applyHy3Tag(window, "hy3_grouped", grouped);
			applyHy3Tag(window, "hy3_tabbed", tabbed);
			break;
		}
